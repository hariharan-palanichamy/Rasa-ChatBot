...
class Zomato:
    ...
    def restaurant_search(self, query="", latitude="", longitude="", cuisines="",price="", limit=5):
        """
        ...
        if str(limit).isalpha() == True:
            raise ValueError('LimitNotInteger')
        headers = {'Accept': 'application/json', 'user-key': self.user_key}
        result = (requests.get(base_url + "search?q=" + str(query) + "&lat=" + str(latitude) + "&lon=" + str(longitude) + "&cuisines=" + str(cuisines), headers=headers).content).decode("utf-8")
        ...
        if json_data['results_found'] != 0:
            df = pd.DataFrame([restaurant['restaurant']['name'],restaurant['restaurant']['location']['address'],float(restaurant['restaurant']['user_rating']['aggregate_rating']),float(restaurant['restaurant']['average_cost_for_two'])] for restaurant in json_data['restaurants'])
            df.columns=['Name','Address','Rating','Price']
            if(price=='High'):
                df=df[df['Price']>700]
            elif(price=='Med'):
                df=df[df['Price']>700 and df.iloc['Price']<300]
            elif(price=='Low'):
                df=df[df['Price']<300]
